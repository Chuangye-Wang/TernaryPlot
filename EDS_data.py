...
class EDS_data():
     ...
     def __init__(self, path, comps):
          ...
          self.data = data
          counts = self.read_counts_data(path)
          self.counts = counts
     ...
     def read_comps_data(self, path):
          ...
          filePath = path
          filename = [file_var + "Comps_" + elem + ".csv" for elem in comps_order]
          data = {}
               
          for i in range(len(self.comps)):
               # Because the sum of composition is 10000, so each data is divided by 100 to get atomic percent value; 
               data[self.comps[i]] = np.array(pd.read_csv(filePath + filename[i])/100)
     ...
     def select_data_area(self, x_min = 0, y_min = 0, x_max = 1, y_max = 1, data_type = 'comps'):
          ...
          x_min = int(len(self.data[self.comps[0]]) * x_min)
          y_min = int(len(self.data[self.comps[0]]) * y_min)
          x_max = int(len(self.data[self.comps[0]]) * x_max)
          y_max = int(len(self.data[self.comps[0]]) * y_max)
          data_array1 = {}
