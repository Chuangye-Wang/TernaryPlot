# -*- coding: utf-8 -*-
"""
Created on Mon Apr 11 11:57:11 2022

@author: ustcw
"""
from pyTernary.ternaryPlot import TernaryPlot
from pyTernary.plots import ternary_plot
import numpy as np
...
def comps_data_plot(data, locs = np.array([]), **kwargs) :
    ...
    tp = TernaryPlot(scale = 100)
    fig, ax = plt.subplots()
    fig.set_size_inches(8, 8*np.sqrt(3)/2)
    
    ternary_plot(tp, x, ax=ax, interval = 10, linewidth = 0, corner_labels = comps)
